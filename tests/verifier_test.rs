...
fn run(dir: &str) -> Result<(), String> {
    ...
    let proof_bytes: Vec<u8> = if proof_path.exists() {
        ...
        let proof_fields_json =
            fs::read_to_string(path.join("proof_fields.json")).map_err(|e| e.to_string())?;
        let proof_fields: Vec<String> =
            serde_json::from_str(&proof_fields_json).map_err(|e| e.to_string())?;
    ...
    if pub_inputs_path.exists() {
        ...
        let pub_inputs_fields_json = fs::read_to_string(path.join("public_inputs_fields.json"))
            .map_err(|e| e.to_string())?;
        let pub_inputs_fields: Vec<String> =
            serde_json::from_str(&pub_inputs_fields_json).map_err(|e| e.to_string())?;
